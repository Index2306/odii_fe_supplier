...
import { getLocationFullAddress } from 'utils/helpers';
import { warehousingActions as actions } from '.';
import { message } from 'antd';

const SERVICE = 'user-service/supplier/supplier-warehousing';
...
export function* getData({ payload }) {
  ...
  try {
    ...
    if (!isEmpty(repos)) {
      if (repos.is_success) {
        repos.data = repos.data.map(item => {
          const ret = { ...item };
          ret.address_final = getLocationFullAddress(item.location_data);
          return ret;
        });
      }
