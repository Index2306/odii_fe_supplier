...
import { distributionsActions as actions } from '.';
...
export function* getData({ payload }) {
  yield delay(500);
  const requestURL = `${PRODUCT_SERVICE}supplier/distribution${payload}`;
  try {
...
export function* getDetail({ payload }) {
  yield delay(500);
  const requestURL = `${PRODUCT_SERVICE}supplier/distribution/${payload}/detail`;
  try {
    // Call our request helper (see 'utils/request')
...
export function* getDetails({ payload }) {
  yield delay(500);
  const requestURL = id =>
    `${PRODUCT_SERVICE}supplier/distribution/${id}/detail`;
  try {
    // Call our request helper (see 'utils/request')
...
export function* updateAndCreate({ payload }) {
  ...
  const requestURL = `${PRODUCT_SERVICE}supplier/product-stock${
    id ? `/${id}` : ''
  }`;
  try {
    // Call our request helper (see 'utils/request')
...
export function* updateQuantity({ payload }) {
  ...
  const requestURL = `${PRODUCT_SERVICE}supplier/product-stock-quantity${
    id ? `/${id}` : ''
  }`;
...
export function* updateState({ payload }) {
  ...
  const requestURL = `${PRODUCT_SERVICE}supplier/product-stock-state/${id}`;
