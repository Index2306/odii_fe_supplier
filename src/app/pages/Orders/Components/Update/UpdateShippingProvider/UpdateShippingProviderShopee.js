...
export default memo(function UpdateShippingProviderShopee({
  order,
  onCancel,
  onFinish,
  ...rest
  ...
  const setDefaultPickupTime = () => {
    if (!currPickupAddress) {
      return;
    }
    const listTimePickup = currPickupAddress.time_slot_list;
    if (
      !currPickupTime ||
      !listTimePickup.some(
        pickupTime =>
          pickupTime.pickup_time_id === currPickupTime.pickup_time_id,
      )
    ) {
      setCurrPickupTime(listTimePickup[0]);
    }
  };
  ...
  const shippingTypes = [
    {
      ...
      description: (
        <span>
          Đơn vị vận chuyển của Shopee <br /> sẽ đến lấy hàng theo thông tin xác
          nhận
        </span>
      ),
  ...
  const descriptions = {
    ...
    SHIPPING_TYPE_PICKUP_STEP_1:
      'Vui lòng chọn một trong các hình thức vận chuyển dưới đây',
    SHIPPING_TYPE_PICKUP_STEP_2: 'Vui lòng chọn địa chỉ và thời gian lấy hàng',
  };

  const getShippingParameter = async () => {
    setLoading(true);
    try {
      const { data } = await request(
        `oms/supplier/orders/${orderId}/get-shipping-parameter`,
      );
      ...
      setShippingParam(data);
      const firtPickupAddress = data?.pickup?.address_list?.[0];
            ...
            <div className="pickup-address__content-wrapper">
              {listPickupAddress.map(pickupAddress => (
                ...
                >
                  <div
                    className={
                      pickupAddress.address_id === currPickupAddress.address_id
