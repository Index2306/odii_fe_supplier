    ...
    if (!isEmpty(dataTimeline)) {
      const temp = dataTimeline.map(item => {
        return {
          ...item,
          title:
            item.metadata?.action_type === 'created'
              ? 'Khởi tạo giao dịch'
              : item.metadata?.action_type === 'supplier_confirm_order'
              ? 'Nhà cung cấp đã xác nhận duyệt đơn hàng'
              : item.metadata?.action_type === 'supplier_confirmed_order'
              ? 'Nhà cung cấp đã xác nhận duyệt đơn hàng'
              : item.metadata?.action_type === 'supplier_rejected_order'
              ? 'Nhà cung cấp đã từ chối duyệt đơn hàng'
              : // : item.metadata?.action_type === 'chief_accountant_confirm'
              // ? 'Giao dịch đã được Kế toán trưởng duyệt'
              // : item.metadata?.action_type === 'chief_accountant_rejected'
