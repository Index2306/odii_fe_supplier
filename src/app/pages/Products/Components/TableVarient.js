...
export default function TableVarient(props) {
  ...
  const handleUpdate = (value, id, product_id) => {
    if (product_id !== idInventory) {
      setIdInventory({ id: product_id });
    }
    if (isNumber(value)) {
      const handleVariations = vari.map(item => ({
        ...item,
        total_quantity: item.id === id ? value : item['total_quantity'],
      }));
  ...
  const columns = React.useMemo(
    () => [
      ...
      //         type="number"
      //         value={text || 0}
      //         onChange={e => handleUpdate(e, record.id, record.product_id)}
      //       />
