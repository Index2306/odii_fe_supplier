...
import { Form, Button, Input, BoxColor } from 'app/components';
...
export default memo(function ListProduct({
  ...
  const selectedVariationMap = useMemo(() => {
    ...
    selectedProducts.forEach(item => {
      variationIds.set(item?.product_variation_id, null);
      productIds.set(item?.product_id, null);
      ids.set(item?.product_variation_id, item?.id);
    });
  ...
  const changeFilter = useCallback(
    debounce(keyword => {
      if (keyword !== '' && keyword.length < 2) {
        console.log('check callback', keyword);
        return;
      }
  ...
  return (
    <ListProductModal
      title="Chọn sản phẩm từ danh sách sản phẩm đã duyệt"
      transitionName=""
      width={920}
      footer={modalFooter}
